const ACI_PROGRESS_MODULES = [
  ...
  },
  {
    ...
    currentState: "Chat-first direction is locked. Backend context isolation, active-comparison handling, explicit vehicle switching, relative references, session buyer-context extraction and the frontend context transport contract are implemented and regression-gated. Comparison pairing now follows fuel and transmission context, exact-variant follow-ups stay scoped, no-match fallbacks expose confirmation choices, and customer-facing answers are guarded against database/internal wording. The public autosuggest backend and upward-opening frontend typeahead support brands, models, variants, features and contextual actions on every active word while preserving the rest of the sentence. Native live spell-check is enabled in the shared composer, and price-city changes use a data-backed picker instead of a chat query. Customer Journey v1 now persists research/evaluation/decision/enquiry readiness, selects one next-best question after every public answer, hides lead CTAs during early research, and exposes an optional quotation handoff only after sufficient narrowing or an explicit enquiry. Compound requests can coordinate one to five named vehicles across comparisons, features, specifications, colours, prices, variants, price breakups, price history, EMI, alternatives and score insights.",
    pending: "Run a deployed portal/session soak test with journey-state telemetry, calibrate readiness thresholds from real customer conversations, add safe streaming status, standardize embedded canvas behavior, render fallback clarification choices, add the visible context chip and recently compared/shortlist tray, add compound handlers for finance eligibility/documents and future ownership-evidence modules as their tools become launch-ready, and finish multilingual answer quality.",
    items: [
...

  {
    ...
    currentState: "The deterministic context ledger, session buyer-context extraction, context trace, unsupported-city honesty, missing-evidence metadata, conditional comparison guidance and data-backed multi-token autosuggest are implemented. Visible context controls, shortlist persistence, lead scoring and the controlled learning engine are not implemented.",
    pending: "Build visible context controls, formal recoveryOptions contract, shortlist state, fair-deal engine, consented learning-event pipeline and offline evaluation workflow. Learning-driven changes must remain versioned, eval-gated and rollback-safe.",
    items: [
...

  {
    ...
    pending: "Add the context chip and clear/change action, then connect personalized suggestions to consented session memory and the recently compared/shortlist state.",
    items: [
