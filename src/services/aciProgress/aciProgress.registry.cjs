const ACI_PROGRESS_MODULES = [
...

  {
    ...
    currentState: "Phase 0 gates are implemented and green for the current backend scope. Standalone Decision Policy / Eligibility constants, decision output contract, policy evaluator, market-judgment isolation audit, provenance/freshness helper eval, degraded-mode inference helper, module-specific policy profile eval, score-insight and similar-cars real-output policy fixtures, score diagnostic language/tool/user-query gates, cross-model score diagnostic eval/tool/bridge smokes, public-chat score smoke, similar-cars filtering/relation/buyer-quality gates, evidence/freshness contract audit, live-bridge decision runtime envelope smoke, and final recommendation eligibility runtime dry-run smoke now exist. Score and similar runtime answers expose decisionPolicy, evidence, provenance, degradedMode, sourceCollections and canUseForFinalRecommendation=false in the live bridge path. Final-decision style user requests now expose a dry-run finalRecommendationEligibility envelope with requestedFinalRecommendation, missing mandatory inputs, blocked reasons and canUseForFinalRecommendation=false; actual final recommendation answers remain disabled.",
    pending: "Add central decision composer/language layer, expand long-tail score/similar/final-intent audit coverage, strengthen no-data recovery wording, improve buyer-context capture, and keep final recommendations disabled until buyer-context recommendation policy and composer are production-ready.",
    items: [
