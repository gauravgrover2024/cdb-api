const ACI_PROGRESS_MODULES = [
  {
    ...
    items: [
      ...
      { key: "nested_feature_acronym_deduplication", name: "Nested full-name/acronym feature deduplication", status: "ready" },
      { key: "vehicle_entity_word_fragment_guard", name: "Vehicle entity word-fragment collision guard", status: "ready" },
      { key: "compound_vehicle_research_router", name: "Data-driven one-to-five-vehicle compound research router", status: "mostly_ready" },
  ...
  },
  {
    ...
    currentState: "Chat-first direction is locked. Backend context isolation, active-comparison handling, explicit vehicle switching, relative references, session buyer-context extraction and the frontend context transport contract are implemented and regression-gated. Comparison pairing follows fuel and transmission context, exact-variant follow-ups stay scoped, no-match fallbacks expose confirmation choices, and customer-facing answers are guarded against database/internal wording. The public autosuggest backend and upward-opening frontend typeahead support brands, models, variants, features and both global and selected-car actions on every active word while preserving the rest of the sentence. Actions include colours, price lists, on-road price, features, specifications, variants, comparisons, EMI, finance, offers, quotation, recommendations and alternatives. Native live spell-check is enabled in the shared composer, and price-city changes use a data-backed picker instead of a chat query. Customer Journey v1 persists research/evaluation/decision/enquiry readiness, selects one next-best question after every public answer, hides lead CTAs during early research, and exposes an optional quotation handoff only after sufficient narrowing or an explicit enquiry. Compound requests coordinate one to five named vehicles across comparisons, features, specifications, colours, prices, variants, price breakups, price history, EMI, alternatives, score insights and Atlas-backed finance eligibility/documents. The frontend now renders selectable clarification choices, staged safe progress wording, a visible clear/change context control, and session-scoped recent/saved vehicle memory.",
    pending: "Run a deployed portal/session soak test with journey-state telemetry, calibrate readiness thresholds from real customer conversations, implement and validate true safe response streaming, standardize every remaining embedded canvas, add future ownership-evidence modules only when their tools become launch-ready, and finish multilingual answer quality. Crash-test, service, resale and TCO evidence remain deliberately deferred for the current scope.",
    items: [
      ...
      { key: "multi_intent_responses", name: "Multi-intent responses", status: "mostly_ready" },
      { key: "compound_research_response", name: "One-to-five-vehicle compound research response", status: "ready" },
      { key: "safe_streaming", name: "Safe first response streaming", status: "planned" },
      { key: "safe_progress_states", name: "Safe staged response progress wording", status: "ready" },
      ...
      { key: "context_chip", name: "Visible context chip near chat bar", status: "ready" },
      { key: "smart_disambiguation_cards", name: "Smart disambiguation cards", status: "mostly_ready" },
      { key: "chat_autosuggest", name: "DB-backed chat autosuggest", status: "ready" },
      { key: "frontend_debug_renderer_decision", name: "Temporary frontend debug renderer decision", status: "pending" },
      { key: "customer_journey_state", name: "Persistent research-to-enquiry journey state", status: "ready" },
      { key: "non_pushy_lead_readiness", name: "Readiness-gated non-pushy enquiry handoff", status: "ready" },
      { key: "contextual_next_actions", name: "Contextual next-best actions", status: "mostly_ready" },
      { key: "recently_compared_shortlist", name: "Session-scoped recently viewed / shortlist tray", status: "ready" },
      { key: "compound_finance_knowledge", name: "Compound finance eligibility and document guidance", status: "ready" },
    ]
  },
  {
  ...
  },
  {
    ...
    currentState: "Context-aware EMI is available. A reviewed Atlas-backed finance knowledge collection now answers document, applicant-type, eligibility, CIBIL, down-payment, tenure, fee and application-process questions without treating them as EMI requests. The capability also participates once per turn in one-to-five-vehicle compound requests and never promises approval or fabricates a live lender offer. Full lender workflow is not ready.",
    pending: "Finalize EMI assumptions and finance-canvas interaction, build the affordability advisor and consented finance lead workflow, then integrate current lender offers only through verified partner data and freshness controls.",
    items: [
      ...
      { key: "finance_knowledge", name: "Atlas-backed eligibility and document guidance", status: "ready" },
      { key: "compound_finance_questions", name: "Compound finance questions", status: "ready" },
...

  {
    ...
    currentState: "The deterministic context ledger, session buyer-context extraction, context trace, unsupported-city honesty, missing-evidence metadata, conditional comparison guidance and data-backed multi-token autosuggest are implemented. The portal now has visible clear/change context controls, selectable clarification cards and session-scoped recent/saved vehicle memory. Lead scoring and the controlled learning engine are not implemented.",
    pending: "Build the formal recoveryOptions contract, account-level consented shortlist persistence, fair-deal engine, consented learning-event pipeline and offline evaluation workflow. Learning-driven changes must remain versioned, eval-gated and rollback-safe.",
    items: [
      { key: "db_backed_autosuggest", name: "DB-backed chat autosuggest", status: "ready" },
      { key: "context_chip", name: "Visible context chip", status: "ready" },
      { key: "clarification_cards", name: "Smart clarification cards", status: "mostly_ready" },
      ...
      { key: "shortlist_tray", name: "Session recent/saved vehicle tray", status: "ready" },
...

  {
    ...
    currentState: "The backend session context ledger, buyer-context extraction and frontend context transport contract are implemented. Generic question-template suggestions remain excluded. A prewarmed public autosuggest API returns ranked brand, model, variant and feature entities, a reusable global action catalog, and selected-car actions. The action catalog covers colours, price lists, on-road price, features, specifications, variants, comparisons, EMI, finance, offers, quotation, recommendations and alternatives, including aliases such as color/colour and price list/pricelist. The shared frontend composer opens suggestions above the chat bar, resolves the word around the cursor, replaces only that segment without duplicating brand/model text, ignores comparison operators such as `vs`, and keeps suggesting later entities, features and actions in the same sentence. Native live spell-check is enabled. Warm service queries are regression-gated below 75 ms.",
    pending: "Use session recent/saved memory to personalize ranked autosuggestions, then add account-level persistence only after consent and identity rules are defined.",
    items: [
      ...
      { key: "global_action_suggestions", name: "Global action suggestions across every active word", status: "ready" },
      ...
      { key: "chatbar_dropdown", name: "Frontend chat-bar autosuggest dropdown", status: "ready" },
      { key: "active_word_typeahead", name: "Cursor-aware typeahead across multi-entity sentences", status: "ready" },
      { key: "live_spellcheck", name: "Native live composer spell-check", status: "ready" },
      { key: "context_chip", name: "Visible selected context chip", status: "ready" },
      { key: "clear_change_context", name: "Clear/change context action", status: "ready" },
      { key: "session_buyer_memory", name: "Session buyer memory", status: "partial" },
      { key: "recently_compared_shortlist", name: "Session recent/saved vehicle tray", status: "ready" }
    ]
  },
...

  {
    ...
    currentState: "The decision-policy, buyer-input, provenance, freshness, degraded-mode, candidate-retrieval, ranking, language and final-readiness layers are implemented and regression-gated for the current backend scope. Score and similar-car answers remain diagnostic unless their own evidence contract permits more. Final recommendation runtime v2 is active only for a complete buyer brief and an evidence-eligible candidate set: it evaluates exact active variants against budget, fuel, transmission and must-have features, returns one buyer-facing pick plus a close alternative and trade-off, and sets finalRecommendationEnabled/canUseForFinalRecommendation only when every in-scope gate passes. Incomplete briefs ask one next-best question instead of forcing a verdict; unsupported cities and missing evidence remain blocked. The current scope uses indexed prices, feature matrices, score profiles and decision profiles. Independent crash applicability is explicitly caveated when unverified, while service-network, resale and TCO evidence are deliberately deferred.",
    latestUpdate: "Final recommendation runtime v2 and its public chat path are active and regression-gated. The launch smoke evaluated 76 exact variants from 23 input models for a complete buyer brief, returned an in-budget winner and runner-up, verified the requested feature evidence, and kept incomplete or unsupported requests blocked.",
    pending: "Run production conversation soak and quality calibration, finish central wording migration, and broaden current-market validation. Crash-test applicability, service-network, ownership-cost, resale and TCO evidence remain deferred for this scope and must be added before those dimensions can influence an unconditional verdict.",
    items: [
      ...
      { key: "final_recommendation_eligibility", name: "Final recommendation eligibility rules", status: "mostly_ready" },
      { key: "final_recommendation_runtime_v2", name: "Evidence-gated final recommendation runtime v2", status: "ready" },
