const ACI_PROGRESS_MODULES = [
...

  {
    ...
    currentState: "The backend session context ledger, buyer-context extraction and frontend context transport contract are implemented. Generic question-template suggestions remain excluded. A prewarmed public autosuggest API returns ranked brand, model, variant and feature entities plus selected-car price, feature, colour, variant and EMI actions. The shared frontend composer now opens those suggestions above the chat bar, resolves the word around the cursor, replaces only that segment without duplicating brand/model text, ignores comparison operators such as `vs`, and keeps suggesting later entities and features in the same sentence. Native live spell-check is enabled. Warm service queries are regression-gated below 75 ms.",
