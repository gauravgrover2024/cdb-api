const ACI_PROGRESS_MODULES = [
  {
    ...
    currentState: "Hybrid meaning-frame routing, DB-backed candidate retrieval, guarded live bridge, comparison context, Answer Composer v1, variant decision enrichment, feature discovery, fuel-filtered feature comparison, and read-model budget discovery are implemented. A deterministic turn ledger now survives every live-bridge response path, records explicit vehicle switches with provenance, keeps Thar distinct from Thar Roxx, resolves last / previous / above references, clears stale comparisons on explicit single-vehicle facts, and emits an auditable context trace. Focused mixed-flow, relative-reference, stale-comparison, public API, buyer-context, and fast buyer-journey gates pass.",
    pending: "Meet the context stress latency budget (latest run: all 64 semantic cases passed, but 468 ms median exceeded the 400 ms limit and 2650 ms p95 exceeded the 1200 ms limit) and verify context persistence through the real portal/session boundary. Also improve Answer Composer verdict language, complete the module/capability registry and full trace/audit contract, reduce prewarm/cold-start time, automate the first 40-customer corpus, and expand final-answer evals.",
    items: [
      ...
      { key: "context_manager_v1", name: "ACI Context Manager v1", status: "mostly_ready" },
...

  {
    ...
    pending: "Phase 6A final composer readiness audit is in progress/ready: final recommendation activation remains disabled until diagnostic/verdict wording is centralized, evidence thresholds are activation-grade, and broader buyer-journey evals pass.",
    items: [
