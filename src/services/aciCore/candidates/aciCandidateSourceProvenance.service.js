...
const buildVehicleQuery = ({ modelKey, make, model, label }) => {
  ...
  if (modelKey) clauses.push({ model_normalized: new RegExp(`^${modelKey.replace(/-/g, "\\s+")}$`, "i") });
  if (model) clauses.push({ model_normalized: new RegExp(`^${model.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i") });
  if (label) clauses.push({ model: new RegExp(label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") });
  if (make && model) {
    clauses.push({
      $and: [
        { brand: new RegExp(make.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") },
        { model_normalized: new RegExp(model.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") },
      ],
    });
  }
...
export const evaluateCandidateSourceProvenance = async ({ rows = [], buyerContext = {} } = {}) => {
  const db = getDb();
  ...
  for (const row of inputRows) {
    ...
    const vehicleQuery = buildVehicleQuery({ modelKey, make, model, label });

    const vehicleDocs = await safeFind(
      vehicles,
      vehicleQuery,
      {
        _id: 1,
    ...
    const priceRowCount = await safeCount(priceRows, { modelKey });
    const featureRowCount = await safeCount(featureMatrix, { modelKey });
    const modelSummaryCount = await safeCount(modelSummary, { modelKey });
