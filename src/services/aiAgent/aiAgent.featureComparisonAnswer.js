...
export const maybeRunAciFeatureComparisonAnswer = async ({
  ...
  return {
    ...
    contextPatch: {
      activeComparison: {
        type: "vehicle_feature_comparison",
        vehicles: models.map((model) => ({
          make: model.make || model.brand || "",
        ...
        fuelFilter: fuelFilter || "",
        features: rows
          .map((row) => row.featureKey || row.key || row.feature || row.displayName || "")
          .filter(Boolean),
        city: context?.anchorCity || context?.selectedVehicle?.city || DEFAULT_CITY,
      },
      ...
      anchorFullModel: "",
      anchorVariant: "",
      anchorCity: context?.anchorCity || context?.selectedVehicle?.city || DEFAULT_CITY,
      selectedColor: null,
    },
    sourceTransparency: {
