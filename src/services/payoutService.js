...
export const generateBankReceivables = ({
  ...
  const receivable = {
    ...
    payout_expected_date: new Date(
      new Date().getTime() + 7 * 24 * 60 * 60 * 1000,
    ).toISOString(), // Default: 7 days from now
    payout_received_date: null,

    // Disbursement details ✅
    payout_disbursement_date: disbursementDate
      ? new Date(disbursementDate).toISOString()
      : new Date().toISOString(),
    payout_remarks:
      disbursementRemarks ||
      `Auto-generated from bank disbursement (${disbursedBankName})`,
  ...
  console.log(
    `📦 Bank receivable created for ${disbursedBankName}: ₹${receivable.payout_amount.toFixed(2)}`,
  );
 ...
 */
export const calculatePayoutsOnDisbursement = async (
  loan,
  disbursementData,
) => {
  const {
  ...
  const receivables = generateBankReceivables({
    ...
    netLoanApprovedAmount:
      ...
      loan.approval_breakup_netLoanApproved ||
      disburseAmount,
    payoutPercentage,
  ...
  console.log(
    `✅ Payouts calculated: ${receivables.length} receivables, ${payables.length} payables`,
  );
  ...
  return {
    ...
    summary: {
      ...
      totalPayable: payables.reduce(
        (sum, p) => sum + (p.payout_amount || 0),
        0,
      ),
      netPayout:
        receivables.reduce((sum, r) => sum + (r.net_payout_amount || 0), 0) -
...
export const validateDisbursementData = (disbursementData) => {
  ...
  if (
    !disbursementData.disburseAmount ||
    parseFloat(disbursementData.disburseAmount) <= 0
  ) {
    errors.push("Disbursement amount must be greater than 0");
  }

  if (
    !disbursementData.disbursedBankName ||
    disbursementData.disbursedBankName.trim() === ""
  ) {
    errors.push("Disbursed bank name is required");
  }

  if (
    disbursementData.payoutPercentage === null ||
    disbursementData.payoutPercentage === undefined
  ) {
    errors.push("Payout percentage is required");
  }

  if (
    disbursementData.payoutPercentage < 0 ||
    disbursementData.payoutPercentage > 100
  ) {
