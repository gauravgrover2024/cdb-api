...
} from "../../services/aciCore/understanding/geminiMeaningFrame.parser.js";
...
const checkCase = ({ frame, expected }) => {
  ...
  if (expected.models && !includesAll([
    ...
    failures.push(`models expected ${JSON.stringify(expected.models)}, got frame models`);
  }

  if (expected.variants && !includesAll([
    ...
    failures.push(`variants expected ${JSON.stringify(expected.variants)}, got frame variants`);
  }
...
const main = async () => {
  if (!getGeminiApiKey()) {
    throw new Error("Gemini API key missing. Set GEMINI_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY, or GOOGLE_API_KEY.");
  }
  ...
  for (const item of cases) {
    ...
    const output = await runAciUnderstandingEngine({
      ...
      parser: parseGeminiMeaningFrame,
    });
    ...
    const summary = {
      ...
      parserErrors: output.parserResult?.errors,
    };
  ...
  console.log(JSON.stringify({
    suite: "ACI Gemini meaning-frame smoke",
